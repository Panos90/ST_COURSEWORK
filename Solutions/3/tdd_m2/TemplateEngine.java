...
public class TemplateEngine {
    ...
    public String evaluate(String templateString, EntryMap entryMap, String matchingMode){
        ...
        if (matchingMode.equals(MM_OPTIMIZATION)){
            ArrayList<Template> del_temp = new ArrayList<>();
            ArrayList<Template> keep_temp = new ArrayList<>();
            for (Template tmpl : sortedTemplates) {
                Template dt = new Template(tmpl.getStartIndex(), tmpl.getEndIndex(),tmpl.getContent());
                del_temp.add(dt);
                Template kt = new Template(tmpl.getStartIndex(), tmpl.getEndIndex(),tmpl.getContent());
                keep_temp.add(kt);
            }
            Result delete_result = instantiate(templateString, del_temp, entryMap.getEntries(), MM_DELETE);
            Result keep_result = instantiate(templateString, keep_temp, entryMap.getEntries(), MM_KEEP);
            if(delete_result.getTemplatesReplaced() > keep_result.getTemplatesReplaced()){
                return delete_result.getInstancedString();
            }
            return keep_result.getInstancedString();
        }
    ...
    private Result instantiate(String instancedString, ArrayList<Template> sortedTemplates, ArrayList<EntryMap.Entry> sortedEntries, String matchingMode){
        ...
        for (Integer i=0; i<sortedTemplates.size(); i++){
            ...
            if(matchingMode.equals(MM_DELETE) && (replaceHappened == Boolean.FALSE)){
